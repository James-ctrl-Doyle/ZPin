...
public:
	...
	void hide();
	// 把提示气泡从屏幕捕获里摘出去。气泡是独立的顶层窗口，宿主窗口被摘出去它不跟着走，
	// 得单独说一声。录屏工具条（ToolVideo）用得上，别的工具条不需要
	void excludeFromCapture();
	// 提示窗口的句柄（还没建时为 0）。工具条要连同提示一起顶回 topmost 带最上面 ——
	// 气泡是独立顶层窗口，宿主工具条被盖住时它也会被一起盖住
	HWND hwnd() const { return tipHwnd; }
public:
