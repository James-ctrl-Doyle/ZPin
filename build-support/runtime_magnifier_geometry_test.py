...
判据全部落在**蓝色准星**上（便携配置里 borderWidth=0，取景框里是纯黑背景，
屏幕上唯一的蓝色来源就是它），无需给屏幕铺图案：

  准星是**单线十字**（x/y 轴画蓝、原点与四象限透明），轴从取景框边缘穿到原点旁：
  * 横向总跨度 == pixW   = srcW * scaleNum
  * 纵向总跨度 == pixImgH = srcH * scaleNum
  这两个量能反推出"每个源像素放大后占多少物理像素"。
  再扫中心那一行 / 那一列，量出轴上**没有蓝色**的那段缺口 —— 那就是透明的原点格，
  也就能算出它覆盖了几个源像素（目标：正好 1 个，且左上角落在取景框正中心）。
...
EXPECT_SCALE = 16.0     # 每个源像素放大后的物理像素数
EXPECT_SRC_W, EXPECT_SRC_H = 25, 15
...
def main():
    ...
    try:
        ...
        im = full.crop(REGION)
        im.save(SHOT)
        print('（已存图 build/logs/mag_geometry.png —— 只有取景框这一小块）')
        ...
        # 原点（透明格）应该正好是光标下那一个源像素：它的左上角在
        # floor(srcW/2)*scaleNum 处（采样窗从 pos - srcW/2 起取，光标落在那个像素的
        # 左边缘），中心再偏半格。⚠ srcW 是奇数时它不等于取景框几何中心。
        exp_cx = int(EXPECT_SRC_W // 2) * EXPECT_SCALE + px_per_cell / 2
        exp_cy = int(EXPECT_SRC_H // 2) * EXPECT_SCALE + px_per_cell / 2
        if abs((gx0 + gx1) / 2 - exp_cx) > 2 or abs((gy0 + gy1) / 2 - exp_cy) > 2:
            print('   !! 原点格没落在光标那个像素上（留白中心 %.1f,%.1f 期望 %.1f,%.1f）'
                  % ((gx0 + gx1) / 2, (gy0 + gy1) / 2, exp_cx, exp_cy))
            ok = False
        else:
            print('✔ 原点格正好落在光标那个源像素上')
    finally:
        try:
            proc.kill()
            proc.wait(timeout=5)
