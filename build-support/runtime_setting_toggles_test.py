...
每行 39 + 1 分隔线），按下表的逻辑坐标 × dpi 就能点到目标那一行的按钮列：
    ...
    开机自启                                       59.5
    管理员模式                                      99.5
    游戏模式                                       159.5   ← 2026-09-25 新增（在管理员模式下面）
    语言                                           219.5
    选区边框                                       259.5
    默认保存位置                                   299.5
    快速保存                                       339.5
    截图历史                                       399.5
    图标                                           439.5   ← 2026-09-26 新增（最后一行）
    按钮列中心 x = 160(菜单宽) + 520(内容宽) - 20(右内边距) - 90(半按钮宽) = 570

    推导：内容区从 y=40 起，每项 = 行(39) + 分隔线(1)，带提示的再 +20
    （管理员模式 / 游戏模式 / 快速保存各带一句提示）。
    ⚠ 顺序由**构造函数的调用顺序**决定，不是 init*Ctrls 的定义顺序。
    ⚠ 改动设置项的顺序或数量后，这里的常量与 runtime_setting_layout_test.py 的
      ROWS 表都要同步，否则点到别的行上、或者布局断言直接失败。
...
ROW_Y = {'autoStart': 59.5, 'admin': 99.5, 'gameMode': 159.5, 'quickSave': 339.5}
BTN_X = 570.0
    ...
    try:
        ...
        if dlg:
            ...
            try:
                ...
                print('   （确认框截图已存 build/logs/confirm_dialog.png）')
