...
⚠ 本目录 2026-09-26 从 ext/icon/ 挪到顶层 icon/ 之后，两处路径跟着改过：
  - Pillow 挂在 build/.pylibs（ZPin 的统一做法，见 build-support/_pylibs.py），
    从 icon/ 往上一级到仓库根再进 build/；
  - resvg 的 node_modules 在 build/.node_modules（项目风格：依赖一律装在 build 下）。
"""
import json
import os
import shutil
